...
class Learner_retrain(object):
    def __init__(self, game):
        self.graph = tf.Graph()
        self.sess = tf.Session(graph=self.graph)
        self.game = game
        self.reset_flag = True # This indicates if hado mixed strategy was reassigned.
                ...
                with tempfile.TemporaryDirectory() as td:
                    ...
                    for t in range(total_timesteps):
                        ...
                        if done:
                            ...
                            if t > self.game.hado_time_step and self.reset_flag:
                                if training_flag == 0:  # defender is training
                                    env.attacker.set_mix_strategy(self.game.hado_str(identity=1, param=self.game.param))
                                    env.attacker.sample_and_set_str()
                                elif training_flag == 1:  # attacker is training
                                    env.defender.set_mix_strategy(self.game.hado_str(identity=0, param=self.game.param))
                                    env.defender.sample_and_set_str()
                                else:
                                    raise ValueError("Training flag is wrong")
